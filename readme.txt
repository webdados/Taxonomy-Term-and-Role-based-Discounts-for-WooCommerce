...
* [FIX] A discount rule that needs the [PRO add-on](https://nakedcatplugins.com/product/taxonomy-term-and-role-based-discounts-for-woocommerce-pro-add-on/?utm_source=wordpress.org&utm_medium=link&utm_campaign=taxonomydiscounts_woocommerce_plugin), while it is not active, stopped every lower priority rule from being applied on the shop and product pages, so a product could show its full price there and still be discounted in the cart
* [FIX] A discount rule that needs the PRO add-on showed the word “error” as its type while the add-on was not active, and editing it silently dropped its settings. It is now shown as not active, with its stored type and an explanation, and cannot be edited or deleted until the add-on is back
...
* [DEV] Discount rules whose type is not registered through the `tdw_discount_types` filter are now skipped when choosing which rule applies, so custom types must be registered no later than `after_setup_theme`. New `is_rule_type_handled()` method to test a type
